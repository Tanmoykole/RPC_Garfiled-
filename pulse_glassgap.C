...
void pulse_glassgap() {
    ...
    for (double d = 0.01; d <= 0.29 + 1e-6; d += 0.02) {
        ...
        for (Long64_t i = 0; i < nentries; ++i) {
            tree->GetEntry(i);
            pulseHeight *= 1e6;  // Scale the pulse height by 10^5
            sum += pulseHeight;
    ...
    graph->SetTitle("Mean Pulse Height vs glass gap;Glass gap (cm);Mean Pulse Height (x10^{6})");
    ...
    graph->GetYaxis()->SetRangeUser(0, 250);  // Adjust Y-axis to fit scaled data

    // Fit function
   // TF1 *fitFunc = new TF1("fitFunc", "[0]*exp(-[1]*x) + [2]", 0.1, 0.5);
    //fitFunc->SetParameters(1e4, 20, 0);  // Adjust initial parameters if needed
